...
# say so. It is also not the copy anybody edits: it is a seed seeded into the data directory,
# reconciled against it on every launch by `kith/infra/seed.py`, which needs `shipped.json`
# in here to do its job.
#
...
datas = [
    ...
    (str(UI_DIST), "ui"),
    # What every previous release shipped as a persona fragment or a bundled skill. It is how
    # `infra/seed.py` recognises a file it wrote in a data directory seeded by a build that no
    # longer exists on the machine — without it, every untouched fragment looks like somebody's
    # own work and stops receiving corrections. Regenerated by `scripts/record_shipped.py`.
    (str(HERE / "shipped.json"), "."),
]
