...
def export(path: str) -> Handoff:
    """Copy a sandbox file out to the handoff folder, keeping its shape.
    ...
    relative = resolved.removeprefix(sandbox.HOME).lstrip("/") or Path(resolved).name
    destination = HANDOFF_DIR / relative
    destination.parent.mkdir(parents=True, exist_ok=True)

    sandbox.copy_out(resolved, destination)
    if not destination.exists():
        raise HandoffError(f"{path} didn't come out of the sandbox.")

    suffix = destination.suffix.lower()
    executable = suffix in EXECUTABLE_SUFFIXES or _has_execute_bit(destination)
    return Handoff(
        sandbox_path=path,
...
def open_with_default_app(host_path: Path) -> None:
    ...
    if target.suffix.lower() in EXECUTABLE_SUFFIXES or _has_execute_bit(target):
        raise HandoffError(
