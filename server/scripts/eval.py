...
context, and the loop.
    ...
    python scripts/eval.py                        # from server/
    docker exec kith-server python scripts/eval.py

Lives here rather than under `kith/services/` because nothing imports it: it is something
you run, not something the server calls. It sat in services/ long enough for its own
instructions to go stale — they said `python -m kith.eval`, a module path that has not
existed since the layering refactor moved it, and nothing caught that because nothing
referenced the file at all.
