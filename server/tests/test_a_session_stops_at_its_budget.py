"""A working session that spends past its token ceiling stops itself, once, with a durable note.
...
The cap's floor is 100k tokens (a single heavy tick can be ~700k), so these tests work in
...
def test_crossing_the_token_cap_rests_the_session_and_leaves_a_note(db, monkeypatch):
    ...
    r._charge_session(conv_id, 60_000)
    assert repo.conversations.is_working(db, conv_id)  # 60k < 100k → still working
    r._charge_session(conv_id, 60_000)  # 120k ≥ 100k → over budget
...
def test_the_meter_only_trips_once(db, monkeypatch):
    ...
    r = _runner_on(db, monkeypatch)
    r._charge_session(conv_id, 200_000)  # blows straight past
    r._charge_session(conv_id, 200_000)  # a second charge must not post a second note
...
def test_re_entering_keep_working_resets_the_meter(db, monkeypatch):
    ...
    r._charge_session(conv_id, 90_000)
...
def test_an_empty_conversation_id_is_a_no_op(db, monkeypatch):
    tuning.apply({"session_token_cap": 100_000})
    r = _runner_on(db, monkeypatch)
    r._charge_session("", 10_000_000)  # a step run with nobody working must not explode
