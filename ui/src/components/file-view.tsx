import { memo, useEffect, useMemo, useState, type ComponentPropsWithoutRef, type ReactNode } from "react";
import ReactMarkdown, { type Components } from "react-markdown";
import remarkGfm from "remark-gfm";
import hljs from "highlight.js/lib/common";
import { Check, Copy, Download, FileCode2, FileText, Loader2, X } from "lucide-react";
...
const BY_EXT: Record<string, Lang> = {
  js: { id: "javascript", label: "JavaScript" }, mjs: { id: "javascript", label: "JavaScript" }, cjs: { id: "javascript", label: "JavaScript" },
  jsx: { id: "javascript", label: "JSX" },
  ts: { id: "typescript", label: "TypeScript" }, tsx: { id: "typescript", label: "TSX" },
  py: { id: "python", label: "Python" }, pyw: { id: "python", label: "Python" },
  rb: { id: "ruby", label: "Ruby" }, go: { id: "go", label: "Go" }, rs: { id: "rust", label: "Rust" },
  java: { id: "java", label: "Java" }, kt: { id: "kotlin", label: "Kotlin" }, swift: { id: "swift", label: "Swift" },
  c: { id: "c", label: "C" }, h: { id: "c", label: "C" },
  cpp: { id: "cpp", label: "C++" }, cc: { id: "cpp", label: "C++" }, cxx: { id: "cpp", label: "C++" }, hpp: { id: "cpp", label: "C++" },
  cs: { id: "csharp", label: "C#" }, php: { id: "php", label: "PHP" },
  sh: { id: "bash", label: "Shell" }, bash: { id: "bash", label: "Shell" }, zsh: { id: "bash", label: "Shell" },
  json: { id: "json", label: "JSON" }, jsonc: { id: "json", label: "JSON" },
  yml: { id: "yaml", label: "YAML" }, yaml: { id: "yaml", label: "YAML" },
  toml: { id: "ini", label: "TOML" }, ini: { id: "ini", label: "INI" }, cfg: { id: "ini", label: "Config" }, conf: { id: "ini", label: "Config" },
  env: { id: "bash", label: "Env" },
  xml: { id: "xml", label: "XML" }, html: { id: "xml", label: "HTML" }, htm: { id: "xml", label: "HTML" }, svg: { id: "xml", label: "SVG" },
  css: { id: "css", label: "CSS" }, scss: { id: "scss", label: "SCSS" }, less: { id: "less", label: "Less" },
  sql: { id: "sql", label: "SQL" }, graphql: { id: "graphql", label: "GraphQL" }, gql: { id: "graphql", label: "GraphQL" },
  diff: { id: "diff", label: "Diff" }, patch: { id: "diff", label: "Diff" },
  dockerfile: { id: "dockerfile", label: "Dockerfile" }, makefile: { id: "makefile", label: "Makefile" },
  lua: { id: "lua", label: "Lua" }, r: { id: "r", label: "R" }, pl: { id: "perl", label: "Perl" }, scala: { id: "scala", label: "Scala" },
};
const BY_NAME: Record<string, Lang> = {
  dockerfile: BY_EXT.dockerfile, makefile: BY_EXT.makefile,
  ".gitignore": { id: "bash", label: "gitignore" }, ".dockerignore": { id: "bash", label: "dockerignore" },
  ".env": BY_EXT.env, "requirements.txt": { id: "bash", label: "requirements" },
};
const MARKDOWN = new Set(["md", "markdown", "mdx"]);

type Kind = { type: "markdown"; label: string } | { type: "code"; lang: string; label: string } | { type: "plain"; label: string };
      ...
      >
        ...
        <div className="flex items-start gap-3 border-b border-border/60 bg-muted/25 px-4 py-3">
          <span className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-lg bg-background/70 text-sky-500 ring-1 ring-border/60">
            {kind.type === "code" ? <FileCode2 className="size-4" /> : <FileText className="size-4" />}
          </span>
          <div className="min-w-0 flex-1">
            <DialogTitle className="truncate text-[15px] leading-tight font-semibold">{heading}</DialogTitle>
            <DialogDescription className="mt-1 flex min-w-0 items-center gap-1.5 text-[11px]">
              <span className="shrink-0 rounded bg-muted px-1.5 py-0.5 font-medium tracking-wide text-muted-foreground">{kind.label}</span>
              {badge}
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            ...
            {content != null ? <IconAction label="Copy" onClick={() => copyText(content)} copyIcon /> : null}
            {onDownload ? <IconAction label="Download" onClick={onDownload} icon={<Download className="size-4" />} /> : null}
            <IconAction label="Close" onClick={() => onOpenChange(false)} icon={<X className="size-4" />} />
          </div>
        </div>
...
function FileBody({ kind, content, view }: { kind: Kind; content: string; view: "rendered" | "source" }) {
  ...
  return (
    <pre className="overflow-x-auto whitespace-pre-wrap break-words p-5 font-mono text-[13px] leading-relaxed">{content}</pre>
  );
}

function IconAction({ label, onClick, icon, copyIcon }: {
  label: string; onClick: () => void; icon?: ReactNode; copyIcon?: boolean;
}) {
...
const MD: Components = {
  ...
  h2: (p) => <h2 className="mt-7 mb-2.5 border-b border-border/50 pb-1.5 text-lg font-semibold tracking-tight first:mt-0" {...p} />,
  ...
  h6: (p) => <h6 className="mt-3 mb-1 text-xs font-semibold uppercase tracking-wide text-muted-foreground first:mt-0" {...p} />,
  p: (p) => <p className="my-3 leading-relaxed first:mt-0 last:mb-0" {...p} />,
  a: ({ href, ...p }) => <a href={href} target="_blank" rel="noreferrer" className="text-kith underline underline-offset-2 hover:text-kith/80" {...p} />,
  ul: (p) => <ul className="my-3 ms-5 list-disc space-y-1 marker:text-muted-foreground" {...p} />,
  ol: (p) => <ol className="my-3 ms-5 list-decimal space-y-1 marker:text-muted-foreground" {...p} />,
  li: (p) => <li className="leading-relaxed" {...p} />,
  blockquote: (p) => <blockquote className="my-3 border-s-2 border-kith/40 ps-4 text-muted-foreground italic" {...p} />,
  hr: () => <hr className="my-6 border-border" />,
  strong: (p) => <strong className="font-semibold" {...p} />,
  em: (p) => <em className="italic" {...p} />,
  img: ({ alt, ...p }) => <img alt={alt} className="my-3 max-w-full rounded-lg border border-border/60" {...p} />,
  table: (p) => (
  ...
  code: ({ className, children, ...props }: ComponentPropsWithoutRef<"code"> & { className?: string }) => {
    ...
    if (!isBlock) {
      return <code className="rounded-md bg-muted px-1.5 py-0.5 font-mono text-[0.85em]" {...props}>{children}</code>;
    }
    const lang = match?.[1];
    return (
  ...
  return (
    <div>
      <div className="flex items-center justify-between border-b border-border/50 bg-muted/30 px-3 py-1.5">
        <span className="font-mono text-[11px] lowercase text-muted-foreground">{label || language || "text"}</span>
        <button
          onClick={copy}
