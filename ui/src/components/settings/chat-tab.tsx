...
/**
 * How he thinks, as opposed to where — sizes, reasoning, and who he is.
 *
...
export function ChatTab({
  ...
  async function save() {
    ...
    try {
      const server = await patchServerConfig({
        numCtx: positiveOr(draft.numCtx, config.numCtx),
  ...
  return (
    <div className="space-y-6">
      <section>
        <div className="mb-3">
          <h2 className="text-sm font-semibold">Room to think</h2>
          <p className="text-muted-foreground text-xs">
            A tick can run sixteen rounds carrying tool output, so context is what keeps him
            coherent to the end of one.
          </p>
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Longest reply (-1 for no limit)">
            <input
              type="number"
              className={inputClass}
