...
export function Companions({
  ...
  /* **The host's position in this tree never changes, and that is the whole point.**
   *
   * This used to `return <>{host}</>` when there were no panels, on the reasoning below — that
   * no companions is not a layout. The reasoning is right about the *pixels* and was wrong about
   * the *tree*: the element at this position went from a Fragment to a `div` the moment the first
   * panel opened, and React reconciles by type, so opening Work or Context on a chat that had
   * neither unmounted the chat underneath it. Its runtime, its thread, its scroll and its
   * half-written message went with it, and it came back by refetching the conversation — which is
   * the same teardown `layout-view` keeps every tab mounted specifically to avoid, and the same
   * one the comment on the full-screen branch below says must not happen.
   *
   * So the wrapper is unconditional and the *column* is what appears: `flex-1` on the host with
   * no siblings is the whole tab, which is what "not a layout" actually asked for. Siblings
   * added and removed after index 0 cannot disturb index 0.
   */
  return (
          ...
          <div
            ...
            onKeyDown={(event) => {
              if (event.key === "ArrowLeft")
                setWidth((was) => clampWidth(was + 16));
